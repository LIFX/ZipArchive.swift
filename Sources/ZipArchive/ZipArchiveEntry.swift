...
public class ZipArchiveEntry {
    ...
    /// - returns:
    public func open(
        encryptionStreamFactory: ZipEncryptionStreamFactory? = nil,
        isLargeFile: Bool = false) throws -> ZipArchiveEntryStream {
        ...
        case .read:
            ...
            if let encryptionStreamFactory = encryptionStreamFactory {
                tmpStream = encryptionStreamFactory.create(stream: archive.stream)
            } else {
...
//            }
        ...
        case .create:
            ...
            if let _ = encryptionStreamFactory {
...
//                let zipCryptoStream = ZipCryptoStream(stream: archive.stream, password: cstr, crc32ForCrypting: crc32, crc32Table: getCRCTable())
                let encryptionStream = encryptionStreamFactory.create(stream: archive.stream)
                tmpStream = encryptionStream
                headerSize = encryptionStream.headerSize
            } else {
                tmpStream = archive.stream
                headerSize = 0
            }
