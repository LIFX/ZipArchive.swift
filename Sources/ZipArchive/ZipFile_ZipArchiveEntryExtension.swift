...
extension ZipArchiveEntry {
    ...
    public func extractTo(filePath: String, overwrite: Bool = false, encryptionStreamFactory: ZipEncryptionStreamFactory? = nil) throws {
        ...
        let unzipStream = try open(encryptionStreamFactory: encryptionStreamFactory)
        defer {
            unzipStream.close()
        }
